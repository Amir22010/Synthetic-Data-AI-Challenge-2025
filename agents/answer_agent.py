...
class AnsweringAgent(object):
    ...
    def build_prompt(self, question_data: Dict[str, str | Any]) -> Tuple[str, str]:
        ...
        sys_prompt1 = """You are an expert-level logical reasoning specialist who carefully analyzes questions and can identify correct answers through systematic thinking.
        ...
        sys_prompt2 = """You are a world-class reasoning expert. Analyze the provided question, choices, and explanation as an expert would.
        ...
        # Include question, choices, and explanation for expert analysis
        tmpl = (
            "EXPERT ANALYSIS ASSIGNMENT:\n\n"
            "QUESTION: {}\n\n"
            "CHOICES: {}\n\n"
            "EXPLANATION: {}\n\n"
        ...
        prompt = tmpl.format(
            ...
            question_data.get("explanation", "No explanation provided")
        )
    ...
    def answer_question(
        ...
        if (
            ...
            # Return empty JSON string for the answer part on error
            empty_json = '{"answer": "A", "reasoning": "Error generating response"}'
            if isinstance(question_data, list):
                return [empty_json] * len(question_data), tl, gt
            else:
                return empty_json, tl, gt
    ...
    def answer_batches(
        ...
        if len(questions) % batch_size != 0:
            ...
            if isinstance(batch_answers, tuple):
                if isinstance(batch_answers[0], list):
                    answers.extend(batch_answers[0])
                else:
                    answers.append(batch_answers[0])
